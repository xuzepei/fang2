...
@interface RCThisMonthViewController ()

@end

@implementation RCThisMonthViewController
...
- (void)updateContent
{
    NSString* urlString = [NSString stringWithFormat:@"%@/xinfang_list.php?apiid=%@&apikey=%@&page=%d&area=%d&type=%d&price=%d&sort=%d&action=nowmonth",BASE_URL,APIID,PWD,self.page,self.quyuSearchId,self.leixingSearchId,self.jiageIndex,self.paixuIndex];
...
- (void)initTableView
{
    ...
    {
        //init table view
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,self.adHeight,[RCTool getScreenSize].width,[RCTool getScreenSize].height - STATUS_BAR_HEIGHT - NAVIGATION_BAR_HEIGHT - self.adHeight)
