...
#define HIGHTLIGHT_COLOR [RCTool colorWithHexString:@"0x01a4f1"]
#define COMMON_COLOR [RCTool colorWithHexString:@"0x7a828d"]

#define PHONE_TAG 111
#define YANZHENG_TAG 112
#define PASS_TAG 113
#define REPASS_TAG 114
#define YUMIMA_TAG 115

@implementation RCPasswordView
...
- (void)updateContent:(int)type token:(NSDictionary*)token
{
    ...
    if(1 == _type)
    {
        if(nil == _phoneTF)
        {
            ...
            _phoneTF.layer.borderColor = [RCTool colorWithHexString:@"0xdadada"].CGColor;
        ...
        if(nil == _yuanmimaTF)
        {
            _yuanmimaTF = [[UITextField alloc] initWithFrame:CGRectMake(16, 220 - 64, [RCTool getScreenSize].width - 32, 40)];
            _yuanmimaTF.tag = YUMIMA_TAG;
            _yuanmimaTF.borderStyle = UITextBorderStyleLine;
            _yuanmimaTF.layer.borderColor = [RCTool colorWithHexString:@"0xdadada"].CGColor;
            _yuanmimaTF.layer.borderWidth = 1;
            _yuanmimaTF.backgroundColor = [UIColor whiteColor];
    ...
    else if(2 == _type)
    {
        ...
        [_yuanmimaTF removeFromSuperview];
        ...
        if(nil == _yanzhengmaTF)
        {
            ...
            _yanzhengmaTF.layer.borderColor = [RCTool colorWithHexString:@"0xdadada"].CGColor;
    ...
    else if(3 == _type)
    {
        ...
        if(nil == _mima0TF)
        {
            ...
            _mima0TF.layer.borderColor = [RCTool colorWithHexString:@"0xdadada"].CGColor;
        ...
        if(nil == _mima1TF)
        {
            ...
            _mima1TF.layer.borderColor = [RCTool colorWithHexString:@"0xdadada"].CGColor;
...
- (IBAction)clickedNextButton:(id)sender
{
    ...
    if(1 == _type)
    {
        ...
        NSString* text = _yuanmimaTF.text;
        if(0 == [text length])
        {
            [RCTool showAlert:@"提示" message:@"请输入原密码！"];
            return;
        }
        else
        {
            if(self.delegate && [self.delegate respondsToSelector:@selector(clickedNextButton:token:)])
            {
                ...
                if(0 == [phone length])
                    phone = [RCTool getUsername];
                
                NSDictionary* token = @{@"phone":phone,@"password":text};
