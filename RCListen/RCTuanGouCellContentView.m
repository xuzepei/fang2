...
@implementation RCTuanGouCellContentView
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSString* zhuangtai = [_item objectForKey:@"tg_title"];
    if([zhuangtai length])
    {
        if(_highlighted)
            [[UIColor whiteColor] set];
        ...
        CGSize size = [zhuangtai sizeWithFont:[UIFont systemFontOfSize:12] constrainedToSize:CGSizeMake(CGFLOAT_MAX, 15) lineBreakMode:NSLineBreakByClipping];
        
        [zhuangtai drawInRect:CGRectMake(310 - size.width, offset_y, size.width, 20)
    ...
    offset_y += 14;
    NSString* teshe = [_item objectForKey:@"tg_limit"];
    if([teshe length])
    {
        NSString* temp = [NSString stringWithFormat:@"剩余时间:%@",teshe];
        if(_highlighted)
            [[UIColor whiteColor] set];
        else
            [[UIColor orangeColor] set];
        
        [temp drawInRect:CGRectMake(86, offset_y, 220, 20)
                 withFont:[UIFont systemFontOfSize:10]
            lineBreakMode:UILineBreakModeTailTruncation];
    }
