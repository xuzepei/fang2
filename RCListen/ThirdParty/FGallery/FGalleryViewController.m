...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    UIBarButtonItem *newBackButton = [[UIBarButtonItem alloc] initWithTitle: NSLocalizedString(@"返回", @"") style: UIBarButtonItemStyleBordered target: nil action: nil];
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"全部图片", @"") style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
...
- (void)positionScroller
{
	...
	if( self.interfaceOrientation == UIInterfaceOrientationPortrait || self.interfaceOrientation == UIInterfaceOrientationPortraitUpsideDown )
	{//portrait
		scrollerRect = CGRectMake( 0, STATUS_BAR_HEIGHT, screenFrame.size.width, screenFrame.size.height - STATUS_BAR_HEIGHT);
	}
	else
	{//landscape
		scrollerRect = CGRectMake( 0, STATUS_BAR_HEIGHT, screenFrame.size.height, screenFrame.size.width - STATUS_BAR_HEIGHT);
	}
...
- (void)positionToolbar
{
	_toolbar.frame = CGRectMake( 0, _scroller.frame.size.height + STATUS_BAR_HEIGHT -kToolbarHeight, _scroller.frame.size.width, kToolbarHeight );
}
...
{
    _isThumbViewShowing = NO;
    [self.navigationItem.rightBarButtonItem setTitle:NSLocalizedString(@"全部图片", @"")];
