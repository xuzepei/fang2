...
@interface RCFangInfoViewController ()

@end

@implementation RCFangInfoViewController
...
- (void)updateContent:(NSDictionary*)item
{
    ...
    NSString* urlString = [NSString stringWithFormat:@"%@/xinfang_content.php?apiid=%@&apikey=%@&id=%@&username=%@&ios=1",BASE_URL,APIID,PWD,id,[RCTool getUsername]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(0 ==  indexPath.section)
    {
        ...
        else if(1 == indexPath.row)
        {
            ...
            if (cell == nil)
            {
                NSArray *objects = [[NSBundle mainBundle] loadNibNamed:@"RCFangInfoJiaGeCell" owner:self options:nil];
            ...
            if(_detailInfo)
            {
                RCFangInfoJiaGeCell* temp = (RCFangInfoJiaGeCell*)cell;
                temp.nameLabel.text = [_detailInfo objectForKey:@"name"];
                temp.jiageLabel.text = [_detailInfo objectForKey:@"price"];
                temp.timeLabel.text = [_detailInfo objectForKey:@"openbatch"];
